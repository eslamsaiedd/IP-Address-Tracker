...
function Tracker() {
  ...
  // Ask for geolocation on mount, then use IP-based lookup
  useEffect(() => {
    const state = routerLocation.state as { searchIp?: string } | null;

    // If re-tracked from history, skip geo prompt
    if (state?.searchIp) {
      setGeoStatus("granted");
      setShowBanner(false);
      fetchIp(state.searchIp);
      window.history.replaceState({}, "");
      return;
    }

    if (!navigator.geolocation) {
      // Browser doesn't support geolocation — just load normally
      setGeoStatus("denied");
      setShowBanner(false);
      fetchIp("");
      return;
    ...
    navigator.geolocation.getCurrentPosition(
      () => {
        // Permission granted — use IP-based lookup (ipgeolocation.io auto-detects)
        setGeoStatus("granted");
        setShowBanner(false);
        fetchIp("");
      },
      () => {
        // Permission denied — still load via IP, just note it
        setGeoStatus("denied");
        setShowBanner(false);
        fetchIp("");
      },
  ...
  return (
    <main className="w-full bg-[#F7F9FB] dark:bg-[var(--second-primary-color)] min-h-[calc(100vh-64px)]">
      ...
      {/* Geo denied notice (subtle, not blocking) */}
      ...
      >
        ...
        {(loading || geoStatus === "asking") && (
          <div
            className="flex justify-center items-center w-full max-w-[800px] h-[120px]"
            role="status"
            aria-label="Loading IP data"
          >
        ...
        {!loading && geoStatus !== "asking" && error && (
          <div className="flex flex-col sm:flex-row gap-3 w-full max-w-[800px]">
            <InfoCards
              className="flex-1"
              show={true}
              fontTitle="text-[32px] sm:text-[40px]"
              title="System Alert"
              info="Invalid IP or Domain"
            ...
            <div className="flex sm:flex-col flex-row gap-3 sm:w-[220px]">
              <InfoCards
                quickTips={true}
                className="flex-1 sm:flex-none border-l-4 border-red-500"
                show={false}
                showCopyButton={false}
                showIconLocation={false}
                showRouter={false}
              ...
              />
              <InfoCards
                className="flex-1 sm:flex-none border-l-4 border-[#2E5BFF]"
                title="Try Again"
                info="New search"
                lastInfo="Enter a valid IPv4, IPv6, or domain"
                show={false}
                quickTips={false}
                showCopyButton={false}
                showIconLocation={false}
        ...
        {!loading && geoStatus !== "asking" && !error && data && (
          <div className="flex flex-col sm:flex-row gap-3 w-full max-w-[800px]">
            <InfoCards
              className="flex-1"
              showCopyButton={true}
              fontTitle="text-[32px] sm:text-[42px]"
              title="Current IP"
              info={data.ip}
              lastInfo=""
              show={false}
            ...
            <div className="flex sm:flex-col flex-row gap-3 sm:w-[220px]">
              <InfoCards
                ...
                showRouter={true}
                title="Location"
                info={`${data.location.city}, ${data.location.region}`}
              ...
              <InfoCards
                ...
                title="ISP Provider"
                info={data.isp}
                lastInfo={`TZ: ${data.location.timezone}`}
                show={false}
                quickTips={false}
                showCopyButton={false}
                showRouter={false}
